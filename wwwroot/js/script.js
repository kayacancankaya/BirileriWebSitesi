	...
	if ($('.price-range-slider').length) {
		$(".price-range-slider").slider({
			...
			slide: function (event, ui) {
				$("input.property-amount").val(ui.values[0] + " - " + ui.values[1] + " TL");
			},
			create: function () { // Ensure it sets correctly on page load
				$("input.property-amount").val(
					$(this).slider("values", 0) + " - " + $(this).slider("values", 1) + " TL"
				);
			}
		});

		// Ensure no "$" is preset in the input
		$("input.property-amount").val(
			$(".price-range-slider").slider("values", 0) + " - " +
			$(".price-range-slider").slider("values", 1) + " TL"
		);
	}
